...
`rel/container.prod.exs` and `rel/container.runtime.exs` are appended to the **image's copies** of `config/prod.exs` and `config/runtime.exs` during build, not to repository files. The production fragment enables `force_ssl`'s `rewrite_on: [:x_forwarded_proto]` with HSTS **at compile time** (Phoenix compiles this plug; a runtime-only override aborts release boot). Compile-environment validation remains enabled. The runtime fragment leaves verified database TLS untouched. Image-only `HTTP_BIND` accepts exactly `127.0.0.1` (default) or `0.0.0.0`; other values fail closed. Compose explicitly fixes loopback. A separately reviewed bridge/proxy override may select wildcard but must restrict direct app ingress and forwarded-header trust. Shared `config/runtime.exs` is untouched; native source releases need the equivalent configuration integrated by Main if this setting is wanted outside the Docker template. Omitting this image-only fragment behind a TLS-terminating proxy would cause HTTPS redirect loops. Ordinary source builds do not acquire this behavior automatically.
